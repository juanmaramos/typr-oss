...
import type { Event, Session } from "@typr/plugin-db";
...
export default function WorkspaceCalendar({
  ...
  return (
    <div
      ref={calendarRef}
      className="grid grid-cols-7 divide-x divide-border h-full grid-rows-6 gap-0"
    >
            ...
            <div className="flex items-center justify-end px-1 text-sm h-8">
              ...
              >
                ...
                <div
                  className={cn(
                    isHighlighted
                      && "bg-destructive rounded-full w-6 h-6 flex items-center justify-center",
                  )}
                >
                  <span
                    className={cn(
                  ...
                  {hiddenCount > 0 && (
                    <Popover>
                      <PopoverTrigger asChild>
                        <div className="text-xs text-muted-foreground rounded py-0.5 cursor-pointer hover:bg-surface-400 mx-1 h-5">
                          {`+${hiddenCount} more`}
                        </div>
                      </PopoverTrigger>
                      <PopoverContent
                        className="w-80 p-4 max-h-52 space-y-1 overflow-y-auto bg-background border m-2 shadow-lg outline-none focus:outline-none focus:ring-0"
                        align="start"
                      >
                        <div className="text-lg font-semibold text-foreground">
                          {format(day, "MMMM d, yyyy", { locale: i18n.locale === "es" ? es : undefined })}
                        </div>
                        ...
                        {dayItems
                          ...
                          .map((item) => (
                            <div
                              key={item.id}
                              className="text-sm hover:bg-surface-400 rounded cursor-pointer transition-colors"
                            >
                              {"calendar_event_id" in item
