...
import { type Human, type Organization } from "@typr/plugin-db";
...
export function ContactView({ userId, initialPersonId, initialOrgId }: ContactViewProps) {
  ...
  return (
    <div className="flex h-full">
      <div className="w-[200px] border-r border flex flex-col">
        <div className="px-3 py-2 border-b border flex items-center justify-between">
          <h3 className="text-xs font-medium text-muted-foreground">Organizations</h3>
          <button
            onClick={() => setShowNewOrg(true)}
            className="p-0.5 rounded hover:bg-surface-400 transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-2">
            <button
              ...
              className={cn(
                "w-full text-left px-3 py-2 rounded-md text-sm flex items-center gap-2 hover:bg-surface-400 transition-colors",
                !selectedOrganization && "bg-muted",
              )}
            >
                ...
                : (
                  <div
                    ...
                    className={cn(
                      ...
                      selectedOrganization === org.id && "bg-muted",
                    )}
                  >
                    <button
                      onClick={() => setSelectedOrganization(org.id)}
                      className="w-full text-left px-3 py-2 text-sm flex items-center gap-2 hover:bg-surface-400 transition-colors rounded-md"
                    >
                    ...
                    <button
                      ...
                      className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-surface-400 transition-all"
                    >
      ...
      <div className="w-[250px] border-r border flex flex-col">
        <div className="px-3 py-2 border-b border flex items-center justify-between">
          ...
          <div className="p-2">
            {displayPeople.map((person) => (
              <button
                ...
                className={cn(
                  "w-full text-left px-3 py-2 rounded-md text-sm hover:bg-surface-400 transition-colors flex items-center gap-2",
                  selectedPerson === person.id && "bg-muted",
                )}
              >
              ...
              : (
                <>
                        ...
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            {selectedPersonData.job_title && (
                              <p className="text-sm text-muted-foreground">{selectedPersonData.job_title}</p>
                            )}
                            {selectedPersonData.email && (
                              <p className="text-sm text-muted-foreground">{selectedPersonData.email}</p>
                            )}
                          ...
                          <button
                            ...
                            className="p-2 rounded-md hover:bg-surface-400 transition-colors"
                          >
                  ...
                  <div className="flex-1 p-6">
                    <h3 className="text-sm font-medium text-muted-foreground mb-4 pl-3">Related Notes</h3>
                    <div className="h-full overflow-y-auto">
                      <div className="space-y-2">
                        {personSessions.length > 0
                          ? (
                            personSessions.map((session) => (
                              <button
                                ...
                                className="w-full text-left p-3 rounded-md border border hover:bg-surface-400/50 transition-colors"
                              >
                                <div className="flex items-center gap-2 mb-1">
                                  <FileText className="h-4 w-4 text-muted-foreground" />
                                  <span className="font-medium text-sm">
                                    {session.title || "New note"}
                                  </span>
                                </div>
                                {session.created_at && (
                          ...
                          : <p className="text-sm text-muted-foreground pl-3">No related notes found</p>}
                      </div>
                    </div>
                  </div>
                </>
    ...
    <div className="flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button
              ...
              size="sm"
              onClick={onCancel}
              className="hover:bg-surface-400 text-foreground/80"
            >
              Cancel
            </Button>
            <Button
              ...
              className="bg-muted hover:bg-surface-400 text-foreground/80"
            >
              Save
            </Button>
          </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="flex flex-col items-center py-6">
          <div className="w-24 h-24 mb-3 bg-secondary rounded-full flex items-center justify-center">
      ...
      <PopoverTrigger asChild>
        <div className="flex flex-row items-center cursor-pointer">
          {organization
            ...
            : <span className="text-muted-foreground text-base">Select organization</span>}
        </div>
      </PopoverTrigger>
        ...
        <div className="flex items-center w-full px-2 py-1.5 gap-2 rounded bg-muted/50 border border">
          ...
          {name.trim() && (
            <button
              ...
              aria-label={t`Save organization`}
            >
