  ...
  return (
    <div className="mt-8">
      <h2 className="mb-4 flex items-center justify-center gap-2 font-semibold">
        ...
        <Trans>Members</Trans>
        <span className="inline-flex items-center justify-center rounded-full bg-muted px-2 py-0.5 text-xs font-medium text-foreground/80">
          {members.length}
        </span>
      </h2>
      ...
      <table className="w-full border-collapse max-w-xs mx-auto">
        <tbody>
          {members.slice(0, 5).map((member) => (
            <tr
              key={member.id}
              className="hover:bg-surface-400 border-b border/50 last:border-b-0"
            >
              <td
