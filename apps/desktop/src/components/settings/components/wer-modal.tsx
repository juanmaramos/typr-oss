  ...
  return (
    <Modal open={isOpen} onClose={onClose} size="full">
      <ModalHeader className="p-6 pb-2">
        ...
        <ModalDescription className="mt-1">
          <Trans>
            ...
            <code className="text-xs font-mono bg-muted p-0.5 rounded">large-v3-turbo</code> model.{" "}
            <a
      ...
      <ModalBody className="p-6 flex flex-col">
        <div className="mb-4">
          <div className="flex justify-between rounded-md p-1 bg-gradient-to-r from-success via-warning to-destructive shadow">
              ...
              <button
                ...
                className={cn(
                  ...
                  selectedCategory === category
                    ...
                    : "text-background hover:bg-background/30",
                )}
              >
        ...
        <div className="flex-1 overflow-y-auto pr-2 space-y-1.5 text-sm">
          {werPerformanceData[selectedCategory].map((lang) => (
            <div
              key={lang.language}
              className="flex justify-between items-center p-2.5 rounded-md bg-muted/50 hover:bg-surface-400 transition-colors"
            >
              <span className="text-sm font-medium">
                <Trans>{lang.language}</Trans>
              </span>
              <span className="font-mono text-muted-foreground">{lang.WER.toFixed(1)}%</span>
            </div>
          ))}
        </div>
      </ModalBody>
