...
export const RatingDisplay = (
  ...
  <div className="flex flex-col items-center px-2">
    <span className="text-[10px] text-muted-foreground uppercase font-medium tracking-wider mb-1.5">{label}</span>
    <div className="flex space-x-1">
      {[...Array(maxRating)].map((_, i) => (
        <Icon
          ...
          className={cn(
            "w-3.5 h-3.5",
            i < rating ? "text-foreground fill-current" : "text-muted-foreground/50",
          )}
...
export interface LLMModel {
  ...
  size: string;
  isCloud?: boolean;
  showInSettings?: boolean; // Optional: Hide from settings UI (defaults to true)
}
...
export interface STTModel {
  ...
  fileName: string;
  isCloud?: boolean;
  supportedLanguages?: string; // Optional: For cloud models to display language capabilities
  hidden?: boolean; // Optional: Hide from UI but keep functional in backend
}
