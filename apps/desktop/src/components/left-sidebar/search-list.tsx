...
import { useTyprSearch } from "@/contexts/search";
...
import { formatRemainingTime } from "@typr/utils/datetime";
...
const highlightText = (text: string, query: string) => {
  ...
  return (
    <>
      ...
      <mark className="bg-warning/20 rounded-sm">
...
export default function SearchList({ matches }: { matches: SearchMatch[] }) {
  const { selectedIndex, query } = useTyprSearch((s) => ({
  ...
  return (
    <div className="h-full space-y-4 px-3 pb-4">
      {sessionMatches.length > 0 && (
        <section>
          <h2 className="font-bold text-muted-foreground mb-1 flex items-center gap-1">
  ...
  return (
    <button
      ...
      className={cn([
        ...
        isActive ? "bg-surface-400" : isSelected ? "bg-primary/10" : "hover:bg-surface-400",
      ])}
    >
      <div className="flex flex-col items-start gap-1">
        <div className="font-medium text-sm truncate">
          {highlightText(session.title || "New note", query)}
        </div>
...
function EventMatch({ match, isSelected, query }: {
  ...
  return (
    <button
      ...
      className={cn([
        "w-full text-left group flex items-start py-2 rounded-lg px-2",
        isSelected ? "bg-primary/10" : "hover:bg-surface-400",
      ])}
    >
      <div className="flex flex-col items-start gap-1">
        ...
        >
          <div className="flex flex-col items-start gap-1">
            <div className="font-medium text-sm line-clamp-1 flex items-center gap-2 w-full">
              ...
              <span className="text-muted-foreground text-xs font-normal ml-auto truncate max-w-[120px]">
      ...
      <ContextMenuTrigger disabled={isActive}>
        ...
        >
          <div className="flex flex-col items-start gap-1 w-full overflow-hidden">
            ...
            <div className="text-xs text-muted-foreground truncate w-full overflow-hidden text-ellipsis whitespace-nowrap">
