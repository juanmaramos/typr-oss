...
// Original organic animation but with proper size constraints
const getRandomValues = (max: number, length: number, baseLength: number) => {
  const values: number[] = [];
  // Constrain max to fit in 32px container (w-8 h-8)
  const constrainedMax = Math.min(max, 16);

  for (let i = 0; i < length - 1; i++) {
    const randomValue = (Math.random() * constrainedMax - constrainedMax / 2) + (baseLength / 100) * constrainedMax;
    // Ensure values stay between 2px and 16px
    values.push(Math.max(2, Math.min(randomValue, 16)));
  }
...
export function DancingSticks(
  { color = "hsl(var(--muted-foreground))", amplitude, size = "default" }: DancingSticksProps,
) {
