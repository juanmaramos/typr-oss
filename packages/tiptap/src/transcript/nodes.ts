...
declare module "@tiptap/core" {
  interface Commands<ReturnType> {
    speaker: {
      ...
      renameSpeakerIdsAfter: (
        position: number,
        oldSpeakerId: string,
        newSpeakerLabel: string,
      ) => ReturnType;

      renameAllSpeakerIndices: (
        oldSpeakerIndex: number,
        newSpeakerLabel: string,
      ) => ReturnType;

      renameSpeakerIndicesAfter: (
        position: number,
        oldSpeakerIndex: number,
        newSpeakerLabel: string,
      ) => ReturnType;
...
const implementCommands = {
  ...
  renameAllSpeakerIds: (oldSpeakerId: string, newSpeakerLabel: string) => ({ tr, dispatch }: CommandProps) => {
    if (!dispatch) {
      return false;
    }
    let updated = false;
  ...
  renameSpeakerIdsAfter:
    (position: number, oldSpeakerId: string, newSpeakerLabel: string) => ({ tr, dispatch }: CommandProps) => {
      if (!dispatch) {
        return false;
      }
      let updated = false;
  ...
  renameAllSpeakerIndices: (oldSpeakerIndex: number, newSpeakerLabel: string) => ({ tr, dispatch }: CommandProps) => {
    if (!dispatch) {
      return false;
    }
    let updated = false;
  ...
  renameSpeakerIndicesAfter:
    (position: number, oldSpeakerIndex: number, newSpeakerLabel: string) => ({ tr, dispatch }: CommandProps) => {
      if (!dispatch) {
        return false;
      }
      let updated = false;
