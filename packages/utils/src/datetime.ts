...
/**
 * Formats time consistently using user's device locale settings
 * This ensures AM/PM casing and 12hr/24hr format matches user's system preferences
 */
...
export function formatDate(date: Date | string): string {
  ...
  if (diffInDays === 0) {
    // Same day - show time using consistent locale formatting
...
/**
 * Categorizes dates using Apple's approach for better i18n compatibility:
 * 1. Today
 * 2. This Week (excluding today)
 * 3. This month (excluding this week)
 * 4. Month Names (June, May, April, etc.)
 */
/**
 * Formats a date in a simplified manner with time:
 * - Today, 3:15pm
 * - Yesterday, 2:30pm
 * - Jul 28, 10:00am
 */
export const formatSimpleDateWithTime = (date: string | null | undefined, t?: string) => {
  if (!date) {
    return i18n._("Unknown date");
  }
  ...
  try {
    ...
    // Format the time using consistent locale formatting
    const timeStr = formatTimeLocale(d);

    if (diffInDays === 0) {
      return i18n._("Today, {time}", { time: timeStr });
    } else if (diffInDays === 1) {
      return i18n._("Yesterday, {time}", { time: timeStr });
    } else {
      const currentYear = now.getFullYear();
      const dateYear = d.getFullYear();
      ...
      if (dateYear === currentYear) {
        const formattedDate = FNS.format(d, "MMM d", { in: tz });
        return i18n._("{date}, {time}", { date: formattedDate, time: timeStr });
      } else {
        const formattedDate = FNS.format(d, "MMM d, yyyy", { in: tz });
        return i18n._("{date}, {time}", { date: formattedDate, time: timeStr });
      }
    ...
    console.error("Error formatting simple date with time:", error);
    return i18n._("Date error");
  }
};

/**
 * Formats a time range for an event:
 * - Today, 2-3pm
 * - Yesterday, 9am-12pm
 * - Jul 28, 10am-12pm
 */
export const formatSimpleDateWithTimeRange = (startDate: string, endDate: string, t?: string) => {
  try {
    ...
    // Format times using consistent locale formatting
    const startTime = formatTimeLocale(startDateObj);
    const endTime = formatTimeLocale(endDateObj);

    // Format date part using the existing helper
    const datePart = formatSimpleDateWithTime(startDate, t).split(", ")[0];

    // If same day event
    if (FNS.isSameDay(startDateObj, endDateObj)) {
      return i18n._("{date}, {startTime}-{endTime}", {
        date: datePart,
        startTime: startTime,
        endTime: endTime,
      });
      ...
      // Multi-day event
      const endDatePart = formatSimpleDateWithTime(endDate, t).split(", ")[0];
      return i18n._("{startDate}, {startTime} - {endDate}, {endTime}", {
        startDate: datePart,
        startTime: startTime,
        endDate: endDatePart,
        endTime: endTime,
    ...
    console.error("Error formatting date with time range:", error);
    return i18n._("Date error");
  }
};

export const formatCategorizedDate = (date: string | null | undefined, t?: string) => {
  ...
  try {
    ...
    const now = new Date();
    // const _startOfDay = FNS.startOfDay(d);
    const startOfToday = FNS.startOfDay(now);
    const startOfWeek = FNS.startOfWeek(now, { weekStartsOn: 0 }); // Sunday
    const startOfMonth = FNS.startOfMonth(now);

    // 1. Today
    if (FNS.isSameDay(d, now, { in: tz })) {
      return i18n._("Today");
    }

    // 2. This Week (excluding today)
    if (d >= startOfWeek && d < startOfToday) {
      return i18n._("This Week");
    }

    // 3. This month (excluding this week)
    if (d >= startOfMonth && d < startOfWeek) {
      return i18n._("This month");
    }

    // 4. Month Names (for older dates)
    const monthName = format(d, "MMMM", { in: tz });
    const currentYear = now.getFullYear();
    const dateYear = d.getFullYear();

    if (dateYear === currentYear) {
      // Same year - just month name
      return monthName;
    } else {
      // Different year - month and year
      return i18n._("{month} {year}", {
        month: monthName,
        year: dateYear.toString(),
      });
    }
  } catch (error) {
    console.error("Error formatting categorized date:", error);
    return i18n._("Date error");
  }
};
