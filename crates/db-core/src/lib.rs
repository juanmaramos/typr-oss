...
pub async fn migrate(
    ...
    tracing::info!("🔧 Starting database migration process");
    let tracking = TrackingSource::new(conn).await?;
    tracing::info!(
        "📊 Migration tracking method: {:?}",
    ...
    if matches!(tracking, TrackingSource::Table) {
        tracing::info!(
            "📝 Creating migration table with SQL: {}",
            MIGRATION_TABLE_SQL
        );
        if let Err(e) = conn.execute(MIGRATION_TABLE_SQL, ()).await {
            tracing::error!("❌ Failed to create migration table: {}", e);
            return Err(e.into());
        }
    }
    ...
    tracing::info!(
        "📈 Migration versions - Current: {}, Latest: {}",
        current_version,
        latest_version
    );
    ...
    if current_version < latest_version {
        ...
        tracing::info!("🔄 Running {} migrations", latest_version - current_version);

        for (index, migration) in migrations.iter().skip(current_version as usize).enumerate() {
            let migration_sql = migration.as_ref();
            let migration_number = current_version + index as i32 + 1;
            tracing::info!(
                "🚀 Executing migration #{}: {}",
                migration_number,
                if migration_sql.len() > 100 {
                    format!("{}...", &migration_sql[..100])
                } else {
            ...
            for statement in statements {
                if let Err(e) = tx.execute(statement, ()).await {
                    tracing::error!("❌ MIGRATION FAILED #{}: {}", migration_number, e);
                    tracing::error!("📜 Failed SQL: {}", statement);
                    return Err(e.into());
                }
            }
            tracing::info!("✅ Migration #{} completed successfully", migration_number);
        }
        ...
        tracing::info!("✅ Database is already up to date");
    }
