...
#[allow(dead_code)]
fn build_enhance_typr_grammar(_s: &Option<Vec<String>>) -> String {
    vec![
...
fn build_enhance_other_grammar(s: &Option<Vec<String>>) -> String {
    match s {
        None => {
            // AUTO MODE: No GBNF constraints - let AI generate natural markdown
            // This allows flexible, content-driven summaries like Granola
            String::new()
        }
        Some(v) if v.is_empty() => {
            // Empty sections also means auto mode
            String::new()
        }
        Some(v) => {
            // TEMPLATE MODE: Use GBNF for structured templates with predefined sections
            build_known_sections_grammar(v)
        }
    }
}
...
fn build_title_grammar() -> String {
    vec![
        r##"root ::= word (" " word)? (" " word)? (" " word)? (" " word)? (" " word)?"##,
        r##"word ::= [A-Za-z0-9] [A-Za-z0-9&'.-]*"##,
    ]
    .join("\n")
}
...
mod tests {
    ...
    fn test_title_grammar() {
        ...
        for (input, expected) in vec![
            ...
            ("A", true),
            ("AI agents reshape software creation", true),
            ("AI-driven software development renaissance", true),
