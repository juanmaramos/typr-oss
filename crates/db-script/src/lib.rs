...
pub mod conversation_to_words {
    fn transform(
        conversation: Vec<typr_listener_interface::ConversationChunk>,
    ) -> Vec<typr_listener_interface::Word> {
        conversation
            .into_iter()
    ...
    pub async fn run(conn: &libsql::Connection) {
        ...
        while let Some(row_result) = rows.next().await.transpose() {
            ...
            let conversations: Vec<typr_listener_interface::ConversationChunk> =
