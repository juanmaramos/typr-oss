...
// AudioStream no longer needs individual pause state since we use global state

// Convenience function to set global pause state for all audio streams
pub fn set_global_pause_state(paused: bool) {
    #[cfg(target_os = "macos")]
    crate::speaker::macos::set_global_pause_state(paused);
}
