...
pub struct CoreAudioSpeakerInput {
    tap: ca::TapGuard,
...
pub struct CoreAudioSpeakerStream {
...
impl CoreAudioSpeakerInput {
    pub fn new(sample_rate_override: Option<u32>) -> Result<Self> {
        ...
        tracing::info!(
            ...
            "[SPEAKER_CAPTURE] CoreAudio output device"
        );
        ...
        extern "C" fn proc(
                ...
                av::AudioPcmBuf::with_buf_list_no_copy(&ctx.format, input_data, None)
            {
                if let Some(data) = view.data_f32_at(0) {
                    ...
                    // Calculate RMS to detect actual audio vs silence
                    ...
                    if count == 100 || count % 1000 == 0 {
                        ...
                        tracing::info!("[SPEAKER_CAPTURE] count={} rms={:.4}", count, rms);
                    }

                    let pushed = ctx.producer.push_slice(data);
                    if pushed < buffer_size {
                        // Only log warnings when not paused - during pause this is expected behavior
                        if !is_globally_paused() {
                            tracing::warn!(
                                "[SPEAKER_CAPTURE] CoreAudio dropped {} samples",
                                buffer_size - pushed,
                            );
                        }
                    }
                ...
                tracing::warn!("[SPEAKER_CAPTURE] CoreAudio empty buffer");
            }
    ...
    pub fn stream(self) -> CoreAudioSpeakerStream {
...
impl Stream for CoreAudioSpeakerStream {
    type Item = f32;
