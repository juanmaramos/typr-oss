        ...
        match init_result {
            Ok((h_event, render_client)) => {
                ...
                loop {
                    ...
                    // Windows WASAPI can have delays during device initialization
                    // Use longer timeout (10s) to prevent premature termination
                    if h_event.wait_for_event(10000).is_err() {
                        error!("timeout error after 10s, stopping capture");
                        break;
                    }
