...
mod macos {
        ...
        for addr in addresses {
            match addr.selector {
                ...
                ca::PropSelector::HW_DEFAULT_OUTPUT_DEVICE => {
                    // For now, we don't need to check if it's a headphone
                    // Just emit the event
                    let _ = event_tx.send(DeviceEvent::DefaultOutputChanged { headphone: false });
                }
                _ => {}
            }
        }
    ...
    pub(super) fn monitor(event_tx: mpsc::Sender<DeviceEvent>, stop_rx: mpsc::Receiver<()>) {
        ...
        tracing::info!("device_monitor_started");
        ...
        tracing::info!("device_monitor_stopped");
    }
}
