...
user_common_derives! {
    pub struct Session {
        ...
        pub auto_enhanced_memo_html: Option<String>,
        #[specta(skip)]
        #[serde(skip)]
        pub conversations: Vec<()>,
        pub words: Vec<typr_listener_interface::Word>,
...
impl Session {
    pub fn from_row(row: &libsql::Row) -> Result<Self, serde::de::value::Error> {
        Ok(Self {
            ...
            auto_enhanced_memo_html: row.get(16).expect("auto_enhanced_memo_html"),
            conversations: vec![],
            words: row
                .get_str(9)
            ...
            source_type: row.get(13).expect("source_type"),
            source_metadata: row.get(14).expect("source_metadata"),
            space_id: row.get(15).expect("space_id"),
            needs_enhance: row.get::<i64>(17).unwrap_or(0) != 0,
        })
    }
