...
async listSupportedModels() : Promise<ModelInfo[]> {
...
async downloadModelBackground(model: SupportedModel) : Promise<null> {
    return await TAURI_INVOKE("plugin:local-llm|download_model_background", { model });
},
...
llmModelStateChanged: LlmModelStateEvent
}>({
llmModelStateChanged: "plugin:local-llm:llm-model-state-changed"
})
...
export type LlmModelState = "NotDownloaded" | { Downloading: { progress: number } } | "Downloaded" | "Loading" | "Ready" | { Error: { message: string } }
export type LlmModelStateEvent = { model_id: string; state: LlmModelState; timestamp: number }
export type ModelInfo = { id: SupportedModel; name: string; provider: string; size: string; description: string; icon: string; show_in_selector: boolean }
export type SupportedModel = "Llama3p2_3bQ4" | "Phi4MiniQ4Km" | "Gemma3_4b" | "Gemma4E4b" | "Qwen3_4bThinkingQ4Km"
