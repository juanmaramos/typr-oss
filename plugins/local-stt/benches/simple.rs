...
fn bench_english_1() -> CustomBenchmarkResult {
    ...
    rt.block_on(async {
        ...
        let client = tauri_plugin_listener::ListenClient::builder()
            .api_base(api_base)
            .api_key("")
            .params(typr_listener_interface::ListenParams::default())
            .build_single();
        ...
        let expected: String = serde_json::from_str::<Vec<typr_listener_interface::Word>>(
            typr_data::english_1::TRANSCRIPTION_JSON,
        )
        .unwrap()
        .into_iter()
