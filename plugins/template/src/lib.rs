...
pub struct State {
    env: typr_template::minijinja::Environment<'static>,
}

impl Default for State {
    fn default() -> Self {
        Self {
            env: typr_template::minijinja::Environment::new(),
        }
    }
}
...
pub fn init<R: tauri::Runtime>() -> tauri::plugin::TauriPlugin<R> {
    ...
    tauri::plugin::Builder::new(PLUGIN_NAME)
        ...
        .setup(|app, _api| {
            ...
            typr_template::init(&mut state.env);
...
mod test {
    ...
    fn render_enhance_system_template<R: tauri::Runtime>(
        ...
        app.render(
            typr_template::PredefinedTemplate::EnhanceSystem,
            serde_json::json!({
    ...
    fn render_enhance_user_template<R: tauri::Runtime>(
        ...
        app.render(
            typr_template::PredefinedTemplate::EnhanceUser,
            serde_json::json!({
                "type": connection.as_ref(),
                "words": [],
                "participants": [],
    ...
    fn test_enhance_system() {
        ...
        assert!(!render_enhance_system_template(&app, "TyprLocal").is_empty());
        assert!(!render_enhance_system_template(&app, "CloudProvider").is_empty());
        assert!(!render_enhance_system_template(&app, "Custom").is_empty());

        assert_ne!(
            render_enhance_system_template(&app, "TyprLocal"),
            render_enhance_system_template(&app, "CloudProvider"),
        );

        insta::assert_snapshot!(render_enhance_system_template(&app, "TyprLocal"), @r###"
        ...
        insta::assert_snapshot!(render_enhance_system_template(&app, "CloudProvider"), @r"
        You are a professional assistant that generates enhanced meetings notes while maintaining accuracy, completeness, and professional terminology in English.
    ...
    fn test_enhance_user() {
        ...
        assert!(!render_enhance_user_template(&app, "TyprLocal").is_empty());
        assert!(!render_enhance_user_template(&app, "CloudProvider").is_empty());
        assert!(!render_enhance_user_template(&app, "Custom").is_empty());

        assert_ne!(
            render_enhance_user_template(&app, "TyprLocal"),
            render_enhance_user_template(&app, "CloudProvider"),
        );

        insta::assert_snapshot!(render_enhance_user_template(&app, "TyprLocal"), @r"
        <participants>
        ...
        insta::assert_snapshot!(render_enhance_user_template(&app, "CloudProvider"), @r###"
        <participants>
