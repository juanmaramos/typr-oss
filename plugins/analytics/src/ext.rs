...
pub trait AnalyticsPluginExt<R: tauri::Runtime> {
    ...
    fn event(
        &self,
        payload: typr_analytics::AnalyticsPayload,
...
impl<R: tauri::Runtime, T: tauri::Manager<R>> crate::AnalyticsPluginExt<R> for T {
    async fn event(
        &self,
        mut payload: typr_analytics::AnalyticsPayload,
    ) -> Result<(), crate::Error> {
        let disabled = {
        ...
        if !disabled {
            let client = self.state::<typr_analytics::AnalyticsClient>();
            client
                .event(payload)
                .await
                .map_err(crate::Error::TyprAnalytics)?;
        }
