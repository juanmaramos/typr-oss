...
pub trait DatabasePluginExt<R: tauri::Runtime> {
    ...
    fn db_get_config(
        &self,
        user_id: impl Into<String>,
    ) -> impl Future<Output = Result<Option<typr_db_user::Config>, crate::Error>>;
    fn db_get_session(
        &self,
        session_id: impl Into<String>,
    ) -> impl Future<Output = Result<Option<typr_db_user::Session>, crate::Error>>;
...
impl<R: tauri::Runtime, T: tauri::Manager<R>> DatabasePluginExt<R> for T {
    ...
    fn db_local_path(&self) -> Result<String, crate::Error> {
        let v = {
            ...
            dir.join("db.sqlite").to_string_lossy().to_string()
        };
    ...
    async fn db_attach(&self, db: typr_db_core::Database) -> Result<(), crate::Error> {
        let state = self.state::<crate::ManagedState>();
        let mut s = state.lock().await;

        let user_db = typr_db_user::UserDatabase::from(db);
        typr_db_user::migrate(&user_db).await?;
    ...
    async fn db_get_session(
        &self,
        session_id: impl Into<String>,
    ) -> Result<Option<typr_db_user::Session>, crate::Error> {
        let state = self.state::<crate::ManagedState>();
        let guard = state.lock().await;

        let db = guard.db.as_ref().ok_or(crate::Error::NoneDatabase)?;
        let session = db
            .get_session(typr_db_user::GetSessionFilter::Id(session_id.into()))
            .await?;
        Ok(session)
    }

    async fn db_upsert_session(&self, session: typr_db_user::Session) -> Result<(), crate::Error> {
        let state = self.state::<crate::ManagedState>();
        let guard = state.lock().await;
    ...
    async fn db_get_config(
        &self,
        user_id: impl Into<String>,
    ) -> Result<Option<typr_db_user::Config>, crate::Error> {
        let state = self.state::<crate::ManagedState>();
        let guard = state.lock().await;
