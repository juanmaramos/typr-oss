...
async fn _sync_calendars(
    user_id: String,
    db_calendars: Vec<typr_db_user::Calendar>,
    system_calendars: Vec<typr_calendar_interface::Calendar>,
    ...
    let calendars_to_delete = {
        let items = db_calendars
            ...
            .cloned()
            .collect::<Vec<typr_db_user::Calendar>>();
...
async fn _sync_events(
    user_id: String,
    db_events_with_session: Vec<(typr_db_user::Event, Option<typr_db_user::Session>)>,
    db_selected_calendars: Vec<typr_db_user::Calendar>,
    system_events_per_selected_calendar: std::collections::HashMap<
        String,
        Vec<typr_calendar_interface::Event>,
    >,
    ...
    let all_system_events: Vec<&typr_calendar_interface::Event> =
        system_events_per_selected_calendar
            .values()
            .flatten()
            .collect();
        ...
        if let Some(ref calendar_id) = db_event.calendar_id {
            if let Some(events) = system_events_per_selected_calendar.get(calendar_id) {
                ...
                if let Some(matching_event) = events.iter().find(|e| e.id == db_event.tracking_id) {
                    let updated_event = typr_db_user::Event {
    ...
    for db_calendar in db_selected_calendars {
        if let Some(fresh_events) = system_events_per_selected_calendar.get(&db_calendar.id) {
            for system_event in fresh_events {
                ...
                let new_event = typr_db_user::Event {
...
fn find_potentially_rescheduled_event<'a>(
    db_event: &typr_db_user::Event,
    system_events: &'a [&typr_calendar_interface::Event],
    db_calendars: &[typr_db_user::Calendar],
) -> Option<&'a typr_calendar_interface::Event> {
...
async fn list_system_calendars() -> Vec<typr_calendar_interface::Calendar> {
    tauri::async_runtime::spawn_blocking(|| {
        let handle = typr_calendar_apple::Handle::new();
...
) -> std::collections::HashMap<String, Vec<typr_calendar_interface::Event>> {
    let now = Utc::now();
...
) -> Result<Vec<typr_db_user::Calendar>, crate::Error> {
    let items = db
...
async fn list_db_events(
    db: &typr_db_user::UserDatabase,
    user_id: impl Into<String>,
) -> Result<Vec<typr_db_user::Event>, crate::Error> {
    let events = db
        ...
        .map_err(|e| crate::Error::DatabaseError(e))?
        .into_iter()
        .collect::<Vec<typr_db_user::Event>>();
...
async fn list_db_events_with_session(
    db: &typr_db_user::UserDatabase,
    user_id: impl Into<String>,
) -> Result<Vec<(typr_db_user::Event, Option<typr_db_user::Session>)>, crate::Error> {
...
struct CalendarSyncState {
    to_delete: Vec<typr_db_user::Calendar>,
    to_upsert: Vec<typr_db_user::Calendar>,
}

#[derive(Debug, Default)]
struct EventSyncState {
    to_delete: Vec<typr_db_user::Event>,
    to_upsert: Vec<typr_db_user::Event>,
    to_update: Vec<typr_db_user::Event>,
}

impl CalendarSyncState {
    async fn execute(self, db: &typr_db_user::UserDatabase) {
