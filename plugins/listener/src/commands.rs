...
#[tauri::command]
#[specta::specta]
pub async fn get_microphone_selection_mode<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
) -> Result<String, String> {
    app.get_microphone_selection_mode()
        .await
        .map_err(|e| e.to_string())
}
...
#[tauri::command]
#[specta::specta]
pub async fn set_microphone_auto<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
) -> Result<(), String> {
    app.set_microphone_auto().await.map_err(|e| e.to_string())
}
...
pub async fn resume_session<R: tauri::Runtime>(app: tauri::AppHandle<R>) -> Result<(), String> {
    tracing::info!("🔄 [RESUME_CMD] Resume session command called");
    app.resume_session().await;
    let final_state = app.get_state().await;
    tracing::info!(
        "🔄 [RESUME_CMD] Final state after resume: {:?}",
        final_state
    );
    match final_state {
        crate::fsm::State::RunningActive { .. } => {
            tracing::info!("✅ [RESUME_CMD] Resume successful");
            Ok(())
        }
        _ => {
            tracing::error!(
                "❌ [RESUME_CMD] Resume failed - unexpected state: {:?}",
                final_state
            );
            Err(crate::Error::ResumeSessionFailed.to_string())
        }
    }
}
...
#[tauri::command]
#[specta::specta]
pub async fn get_pipeline_status<R: tauri::Runtime>(
    app: tauri::AppHandle<R>,
) -> Result<crate::PipelineStatus, String> {
    Ok(app.get_pipeline_status().await)
}
