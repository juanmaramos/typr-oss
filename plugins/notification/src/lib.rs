...
pub struct State {
    ...
    detector: typr_detect::Detector,
    lifecycle_handle: Option<tokio::task::JoinHandle<()>>,
    lifecycle_signal_tx: Option<tokio::sync::mpsc::UnboundedSender<ext::OrchestratorMessage>>,
    listener_event_id: Option<tauri::EventId>,
}
...
pub fn init<R: tauri::Runtime>() -> tauri::plugin::TauriPlugin<R> {
    ...
    tauri::plugin::Builder::new(PLUGIN_NAME)
        ...
        .setup(|app, _api| {
            tracing::info!("🚀 Notification plugin initializing...");
            let state = SharedState::default();
            app.manage(state);

            // Register the meeting notification category with action buttons (macOS).
            #[cfg(target_os = "macos")]
            typr_notification2::register_meeting_category();

            let detect_enabled = app.get_detect_notification().unwrap_or(false);
            tracing::info!("🔔 Meeting detection setting: {}", detect_enabled);

            if detect_enabled {
                tracing::info!("✅ Starting meeting detection...");
                if let Err(e) = app.start_detect_notification() {
                    tracing::error!("❌ start_detect_notification_failed: {:?}", e);
                } else {
                    tracing::info!("🎯 Meeting detection started successfully!");
                }
            } else {
                tracing::warn!("⚠️  Meeting detection is DISABLED - toggle it ON in settings");
            }
